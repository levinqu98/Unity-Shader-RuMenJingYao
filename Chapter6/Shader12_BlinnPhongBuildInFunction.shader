...
Shader "Unity Shaders Book/Chapter 6/Specular Vertex-Level" {
	...
	SubShader {
		...
		v2f vert(a2v v) {
			...
			o.worldNormal = UnityObjectToWorldNormal(v.normal); // 使用内置函数将顶点法线从模型空间转到世界空间
		...
		fixed4 frag(v2f i) : SV_Target {
			...
			fixed3 worldLightDir = normalize(UnityWorldSpaceLightDir(i.worldPos)); // 使用内置函数获取世界空间的光源方向
